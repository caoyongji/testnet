...
test(5) -> 
    ...
    {Ctx3, _} = channel_solo_close_tx:make(constants:master_pub(), Fee, SignedScriptPubKey, [ScriptSig], Trees3),
    Stx3 = keys:sign(Ctx3),
    absorb(Stx3),
    %mine_blocks(1),
    timer:sleep(500),
...
test(10) ->
   %spend shares with channel, with solo_close
    ...
    SignedScriptPubKey = testnet_sign:sign_tx(ScriptPubKey, NewPub, NewPriv), 
    ScriptSig = compiler_chalang:doit(<<" int 0 int 1 ">>),
    {Ctx4, _} = channel_solo_close_tx:make(constants:master_pub(), Fee, SignedScriptPubKey, [ScriptSig], Trees4),
    Stx4 = keys:sign(Ctx4),
    absorb(Stx4),
    {Trees5, _, _} = tx_pool:data(),
    Accounts5 = trees:accounts(Trees5),
...
test(12) ->
    ...
    ScriptSig = compiler_chalang:doit(<<" int 0 int 1 ">>),
    ScriptSig2 = compiler_chalang:doit(<<" int 0 int 2 ">>),
    {Ctx3, _} = channel_solo_close_tx:make(constants:master_pub(), Fee, SignedScriptPubKey, [ScriptSig, ScriptSig2], Trees3),
    Stx3 = keys:sign(Ctx3),
    absorb(Stx3),
    timer:sleep(500),
...
test(15) ->
    ...
    {Ctx3, _} = channel_solo_close_tx:make(NewPub, Fee, SignedScriptPubKey, [ScriptSig], Trees3),
    Stx3 = testnet_sign:sign_tx(Ctx3, NewPub, NewPriv),
    absorb(Stx3),
    timer:sleep(200),
