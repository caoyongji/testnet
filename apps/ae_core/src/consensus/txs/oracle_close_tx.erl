...
%% If there is a lot of open orders for one type of share in an oracle 
%% for a long enough period of time, then this transaction can be done.
%% This ends betting in the market.
%% The fee that was used to start the oracle is the final bet included. 
%% It bets against the winning outcome.
...
doit(Tx, Trees, NewHeight) ->
    Accounts = trees:accounts(Trees),
    Acc = accounts:update(Tx#oracle_close.from, Trees, -Tx#oracle_close.fee, 
                          Tx#oracle_close.nonce, NewHeight),
    NewAccounts = accounts:write(Accounts, Acc),
    OID = Tx#oracle_close.oracle_id,
    Oracles = trees:oracles(Trees),
    {_, Oracle, _} = oracles:get(OID, Oracles),
    true = oracles:starts(Oracle) =< NewHeight,
    %% if the volume of orders in the oracle is too low, 
    %% then set the oracle:type to 3.
