...
%% The shares are stored by share id. The id of a share determines
%% it's difficulty. You can own either a negative, positive,
%% or zero amount of each type of share. Shares are transferable.
...
-record(share, {
          id,
          amount,
          %% We need to keep a record of when it was modified
          %% so that users can get paid for having shares.
          modified
         }).
...
get_paid(Share, Height, Trees) ->
    ...
    %% the rate at which shares convert to tokens
    SharesConversion = governance:get_value(shares_conversion, Governance),
    get_paid2(OldHeight, Height, SDiff, Share#share.amount, 0, SharesConversion).

%% For every difficulty in the range, see if we are over or under.
%% Positive shares get paid for being over, negative get paid for being under.
